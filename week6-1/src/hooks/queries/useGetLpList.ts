...
function useGetLpList({cursor, search, order, limit}: PaginationDto) {
  return useQuery({
    ...
    // 이 시간 동안은 캐시된 데이터를 그대로 사용함. 컴포넌트가 마운트되거나, 창에 포커스 들어오는경우도 재요청 X
    // 5분 동안 기존 데이터를 그대로 활용해서 네트워크 요청을 줄인다.
    staleTime: 1000*60*5, //5분은 이렇게 씀

    // 사용되지 않는(=비활성상태) 쿼리 데이터가 캐시에 남아있는 시간
    // staleTime이 지나고 데이터가 신선하지 않더라도, 일정 시간 동안 메모리에 보관
    // 그 이후에 해당 쿼리가 전혀 사용되지 않으면 gcTime이 지난 후에 제거한다. (*제거 = garbage collection)
    // ex-10분 동안 사용되지 않으면 해당 캐시 데이터가 삭제되어, 다시 요청 시 새 데이터를 받아오게 한다.
    gcTime:100*60*10, //10분

    //그 외에 많이 쓰는 것
    // enabled 조건에 따라 쿼리를 실행 여부 제어
    // enabled: true면 동작, false면 동작 X
    ...
    //컴포넌트가 렌더링될 때 빈 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해주는 것
    ...
    select: (data) => data.data.data, //data.data.data에서 data 덜 쓸 수 있게 됨
  });
}
