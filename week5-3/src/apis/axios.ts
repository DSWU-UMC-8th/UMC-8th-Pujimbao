...
let refreshPromise:Promise<string> | null = null; //let으로 바꿈
...
axiosInstance.interceptors.response.use(
  ...
  async(error) => {
    ...
    // 401 에러면서, 아직 재시도 하지 않은 경우 요청 처리
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      // refresh 엔드포인트 401 에러가 발생한 경우 (Unathorized), 중복 재시도 방지를 위해 로그아웃 처리
      ...
      if(!refreshPromise) {
        ...
        refreshPromise = (async() => {
          ...
          .catch((error) => {
            const {removeItem: removeAccessToken} = useLocalStorage(LOCAL_STORAGE_KEY.accessToken);
            const {removeItem: removeRefreshToken} = useLocalStorage(LOCAL_STORAGE_KEY.refreshTokenToken);
      ...
      // 진행 중인 refreshPromise가 비동기이므로, 해결될 때까지 기다림
